...
func getSecretsYAML(deployOptions *types.DeployOptions) (map[string][]byte, error) {
	...
	if secret := privateKotsadmRegistrySecret(deployOptions.Namespace, deployOptions.KotsadmOptions); secret != nil {
...
func EnsurePrivateKotsadmRegistrySecret(namespace string, kotsadmOptions types.KotsadmOptions, clientset kubernetes.Interface) error {
	...
	if err != nil {
		...
		secret := privateKotsadmRegistrySecret(namespace, kotsadmOptions)
		if secret == nil {
			return nil
		}
